...
Процедура ПриОткрытии(Отказ)
    // Запускаем отложенный пересчет через 1 секунду
    ПодключитьОбработчикОжидания("ПересчитатьСуммуОтложенно", 1.0, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ПересчитатьСуммуОтложенно()
    Объект.СуммаИтого = Объект.ТабНоменклатура.Итог("Сумма");
    Объект.СуммаИтогоПрописью = РаботаСДокументами.ИтоговоеЧислоПрописью(Объект.СуммаИтого);
КонецПроцедуры
...
&НаСервере
Процедура ПослеЗаполненияНаОснованииНаСервере(ОснованиеСсылка)
    // Пересчитываем итоговую сумму и сумму прописью после заполнения на основании
    Объект.СуммаИтого = Объект.ТабНоменклатура.Итог("Сумма");
    Объект.СуммаИтогоПрописью = РаботаСДокументами.ИтоговоеЧислоПрописью(Объект.СуммаИтого);
КонецПроцедуры
